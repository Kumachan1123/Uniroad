...
SETexture::SETexture()
	...
	, m_position(DirectX::SimpleMath::Vector2(0.1f, 0.66f)) // 位置
	, m_size(DirectX::SimpleMath::Vector2(0.125f, 0.1f)) // サイズ
	, m_frameRows(1) // 画像の行数
	, m_frameCols(1) // 画像の列数
{
}
...
*	@return なし
*/
void SETexture::Initialize(CommonResources* resources, int width, int height)
{
	// 共通リソースをセット
	...
	// 画像を設定
	m_pImage->SetTexture(resources->GetTextureManager()->GetTexture("SEText"));
	// シェーダーバッファサイズを設定
...
/*
*	@brief 更新
*	@details 「SE」テクスチャクラスの更新を行う
*	@param elapsedTime 経過時間
*	@return なし
*/
void SETexture::Update(float elapsedTime)
{
	// 未使用警告非表示
...
/*
*	@brief 描画
*	@details 「SE」テクスチャクラスの描画を行う
*	@param なし
*	@return なし
*/
void SETexture::Render()
{
	// 画像を描画
...
*	@return なし
*/
void SETexture::UpdateConstantBuffer()
{
	// 名前空間を使用
