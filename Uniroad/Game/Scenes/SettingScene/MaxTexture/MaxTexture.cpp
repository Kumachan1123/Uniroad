...
*/
#include "pch.h"
#include "MaxTexture.h"
/*
*	@brief コンストラクタ
*	@details 「MAX」テクスチャクラスの初期化を行う
*	@param なし
*	@return なし
*/
MaxTexture::MaxTexture()
	...
	, m_size(DirectX::SimpleMath::Vector2(0.15f, 0.1f)) // ロゴのサイズ
	, m_frameRows(1) // 画像の行数
	, m_frameCols(1) // 画像の列数
{
}
...
*	@return なし
*/
void MaxTexture::Initialize(CommonResources* resources, int width, int height)
{
	// 共通リソースをセット
	...
	// 画像を設定
	m_pImage->SetTexture(resources->GetTextureManager()->GetTexture("Max"));
	// シェーダーバッファサイズを設定
...
/*
*	@brief 更新
*	@details 「MAX」テクスチャクラスの更新を行う
*	@param elapsedTime 経過時間
*	@return なし
*/
void MaxTexture::Update(float elapsedTime)
{
	// 未使用警告非表示
...
/*
*	@brief 描画
*	@details 「MAX」テクスチャクラスの描画を行う
*	@param なし
*	@return なし
*/
void MaxTexture::Render()
{
	// 画像を描画
...
*	@return なし
*/
void MaxTexture::UpdateConstantBuffer()
{
	// SimpleMathの名前空間を使用
