...
void PuyoGrid::Initialize(CommonResources* resources, int width, int height)
{
	// 共通リソースをセット
	...
	// 画像を設定
	m_pImage->SetTexture(resources->GetTextureManager()->GetTexture("Puyo_Grid"));
	// シェーダーバッファサイズを設定
...
void PuyoGrid::Update(float elapsedTime)
{
	// 定数バッファを更新
	UpdateConstantBuffer();
}

void PuyoGrid::Render()
{
	// 画像を描画
...
void PuyoGrid::Finalize()
{}

void PuyoGrid::UpdateConstantBuffer()
{
	// SimpleMathの名前空間を使用
